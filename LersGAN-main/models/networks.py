...
def weights_init(m):
    classname = m.__class__.__name__
    if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('BatchNorm2d') != -1):
        if classname.find('Conv') != -1:
            m.weight.data.normal_(0.0, 0.02)
        elif classname.find('BatchNorm2d') != -1:
            m.weight.data.normal_(1.0, 0.02)
...
class Unet_resize_conv_old(nn.Module):
...
class SpatialAttentionModule(nn.Module):
    def __init__(self, in_channels):
        ...
        # 使用1x1卷积来生成查询（query）、键（key）和值（value）
        self.query_conv = nn.Conv2d(in_channels, 1, kernel_size=1)
        self.key_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1)
        self.value_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1)

    def forward(self, x):
        batch_size, in_channels, height, width = x.size()
        query = self.query_conv(x).view(batch_size, 1, -1)  # [batch_size, 1, height*width]
        key = self.key_conv(x).view(batch_size, in_channels, -1)  # [batch_size, in_channels, height*width]
        value = self.value_conv(x)  # [batch_size, in_channels, height, width]
        query = query.permute(0, 2, 1)  # [batch_size, height*width, 1]
        attention_scores = torch.bmm(key, query)  # [batch_size, in_channels, 1]
        attention_weights = F.softmax(attention_scores, dim=1)  # [batch_size, in_channels, 1]
        attention_weights = attention_weights.view(batch_size, in_channels, 1, 1)  # [batch_size, in_channels, 1, 1]
        out = value * attention_weights.expand_as(value)  # 应用注意力权重
...
import torch.nn as nn
import torch.nn.functional as F
from model.utils.dca import DCA  # 确保DCA模块正确导入
...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        self.spatial_attention1 = SpatialAttentionModule(32)
        ...
        self.spatial_attention2 = SpatialAttentionModule(64)
        ...
        self.spatial_attention3 = SpatialAttentionModule(128)
        ...
        self.spatial_attention4 = SpatialAttentionModule(256)
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            ...
            conv1 = self.bn1_2(self.LReLU1_2(self.conv1_2(x)))
            x = self.max_pool1(conv1)
            x = self.spatial_attention1(x)  # 应用空间注意力

            x = self.bn2_1(self.LReLU2_1(self.conv2_1(x)))
            conv2 = self.bn2_2(self.LReLU2_2(self.conv2_2(x)))
            x = self.max_pool2(conv2)
            x = self.spatial_attention2(x) # 应用空间注意力

            x = self.bn3_1(self.LReLU3_1(self.conv3_1(x)))
            conv3 = self.bn3_2(self.LReLU3_2(self.conv3_2(x)))
            x = self.max_pool3(conv3)
            x = self.spatial_attention3(x) # 应用空间注意力

            x = self.bn4_1(self.LReLU4_1(self.conv4_1(x)))
            conv4 = self.bn4_2(self.LReLU4_2(self.conv4_2(x)))
            x = self.max_pool4(conv4)
            x = self.spatial_attention4(x) # 应用空间注意力
            
            x1, x2, x3, x4 = self.DCA([conv1, conv2, conv3, conv4])
            ...
            conv5 = F.upsample(conv5, scale_factor=2, mode='bilinear')
            conv4 = conv4*gray_4 if self.opt.self_attention else conv4
            up6 = torch.cat([self.deconv5(conv5), x4], 1)
            x = self.bn6_1(self.LReLU6_1(self.conv6_1(up6)))
            conv6 = self.bn6_2(self.LReLU6_2(self.conv6_2(x)))

            conv6 = F.upsample(conv6, scale_factor=2, mode='bilinear')
            conv3 = conv3*gray_3 if self.opt.self_attention else conv3
            up7 = torch.cat([self.deconv6(conv6), x3], 1)
            x = self.bn7_1(self.LReLU7_1(self.conv7_1(up7)))
            conv7 = self.bn7_2(self.LReLU7_2(self.conv7_2(x)))

            conv7 = F.upsample(conv7, scale_factor=2, mode='bilinear')
            conv2 = conv2*gray_2 if self.opt.self_attention else conv2
            up8 = torch.cat([self.deconv7(conv7), x2], 1)
            x = self.bn8_1(self.LReLU8_1(self.conv8_1(up8)))
            conv8 = self.bn8_2(self.LReLU8_2(self.conv8_2(x)))

            conv8 = F.upsample(conv8, scale_factor=2, mode='bilinear')
            conv1 = conv1*gray if self.opt.self_attention else conv1
            up9 = torch.cat([self.deconv8(conv8), x1], 1)
