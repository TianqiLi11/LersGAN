...
from model.utils.ceam_utils import *
...
class ChannelAttention(nn.Module):
    def __init__(self, in_features, out_features, n_heads=1) -> None:
        ...
        self.k_map = depthwise_projection(in_features=out_features, 
    ...
    def forward(self, x):
        ...
        c = q.shape[2]
        scale = c ** -0.5                     
        q = q.reshape(b, hw, self.n_heads, c // self.n_heads).permute(0, 2, 1, 3).transpose(2, 3)
        k = k.reshape(b, hw, self.n_heads, c_k // self.n_heads).permute(0, 2, 1, 3).transpose(2, 3)
        v = v.reshape(b, hw, self.n_heads, c // self.n_heads).permute(0, 2, 1, 3).transpose(2, 3)
        att = self.sdp(k, q, v, scale).permute(0, 3, 1, 2).flatten(2)
...
class Mynew_Attention(nn.Module):
    def __init__(self, in_features, out_features, n_heads=4) -> None:
        super().__init__()
        self.n_heads = n_heads
    ...
    def forward(self, x):
        ...
        k = k.reshape(b, hw, self.n_heads, c // self.n_heads).permute(0, 2, 1, 3)
        v = v.reshape(b, hw, self.n_heads, c_v // self.n_heads).permute(0, 2, 1, 3)
        att = self.out(q, k ,v, scale).transpose(1, 2).flatten(2)    
        x = self.projection(att)
        return x
...
class CCSABlock(nn.Module):
    ...
    def channel_attention(self, x):
        ...
        x_cin = self.cat(*x_c)
        x_in = [[x_cin, k, x_cin] for k in x_c]
