...
class MyNewAttention(nn.Module):
    def __init__(self, q_dim, v_dim, k_dim, hidden_dim):
    ...
    def forward(self, q, k, v, scale):
        q_l = self.W_q(q)  # 线性变换 Q
        v_l = self.W_v(v)  # 线性变换 V
        k_l = self.W_k(k)  # 线性变换 K

        # 加性融合 Q 和 K
        qv_sum = q_l + k_l  
        qv_sum = torch.tanh(qv_sum)  # 非线性激活
        
        # 点积计算 Q 和 K 
        qv_dp = torch.matmul(q, k.transpose(-2, -1))  # 计算 Q 和 K 的点积
        qv_dp = qv_dp.view(q.size(0), q.size(1), -1)  # 调整 qv_dp 的大小

        integrated = qv_sum + qv_dp  

        # integrated与 V 点乘
        energy = torch.einsum('bhd,bhd->bh', integrated, v)  
        energy = energy * scale 

        # 应用 softmax 获取注意力权重并输出
